...
UNIT_DEST="${UNIT_DEST:-/etc/systemd/system/is4brag-index-worker.service}"
ENV_DEST="${ENV_DEST:-/etc/is4brag/worker.env}"
...
if [[ ! -e "$ENV_DEST" ]]; then
    sed "s|/srv/is4brag/data|$BASE|g" \
        "$REPO_ROOT/systemd/worker.env.example" | sudo tee "$ENV_DEST" >/dev/null
...
sed \
    ...
    "$REPO_ROOT/systemd/is4brag-index-worker.service" | sudo tee "$UNIT_DEST" >/dev/null
sudo chmod 0644 "$UNIT_DEST"
sudo systemctl daemon-reload
sudo systemctl enable is4brag-index-worker.service
echo "Installed. Start after editing $ENV_DEST:"
echo "  sudo systemctl start is4brag-index-worker.service"
