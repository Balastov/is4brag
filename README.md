...
4 непересекающихся раздела проектной документации из Confluence `conf-metro.ibs.ru`:
- **Стадии проекта** (8 484) — проектные решения, опросные листы, BPMN, спецификации требований к подсистемам
...
Разделы задаются корневыми страницами и обходятся по потомкам, поэтому они не
должны вкладываться друг в друга: страница хранится по `page_id` один раз, и
пересечение отняло бы её у одного из разделов.
